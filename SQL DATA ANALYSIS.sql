...
## checking with date wise
select order_id,order_Date,sales,profit
from orders_data
order by order_date desc
limit 5;

## if there are same dates, then sort by profit 
select order_id,order_Date,sales,profit
from orders_data
order by order_date, profit desc
limit 5;

## give top 5 sales order
select *
from orders_data
order by sales desc
limit 5;

## if new column to be created
...
select *
from orders_data
where region ='central' and  category = 'technology' and sales >1000;
...
order by quantity;
### for list of things to filter use in
select *
from orders_data
...
select count(*) as no_records from orders_data;
...
select count(distinct category), count(distinct city)
from orders_data;
...
inner join returns_data as r on o.order_id= r.order_id
where o.order_id= 'CA-2020-109806';

select *
from orders_data as o
left join returns_data as r on o.order_id = r.order_id;
...
### in case when statements the order goes from top to bottom
...
select *
,left(ORDER_ID,2) as ORDER_length
from orders_data;

select *
,right(ORDER_ID,6) as ORDER_length
from orders_data;
...
select *
,replace(customer_name,'C', 'M') as new_cust_name
from orders_data;
...
from orders_data;
select order_id,order_date,
month(order_date) as order_mn
from orders_data;

select order_id,order_date,
month(order_date) as order_month,year(order_date) as Yr, sum(sales)
from orders_data
group by order_month, Yr
order by order_date;

select order_id, order_date,
monthname(order_date) AS order_mnth_name
from orders_data
order by order_mnth_name;
